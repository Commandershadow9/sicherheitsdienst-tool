...
export function useListParams(defaults?: Partial<{ page: number; pageSize: number; sortBy: string; sortDir: 'asc'|'desc' }>) {
  ...
  }, [sp, defaults?.page, defaults?.pageSize, defaults?.sortBy, defaults?.sortDir])

  const update = (patch: Partial<{ page: number; pageSize: number; sortBy: string; sortDir: 'asc'|'desc'; filters: Record<string,string|undefined> }>) => {
