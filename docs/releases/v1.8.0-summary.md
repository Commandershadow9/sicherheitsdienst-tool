...
v1.8.0 ist der bisher größte Release seit v1.0.0 und bringt das **Intelligente Replacement System** - ein KI-basiertes Scoring-System für optimale Mitarbeiter-Empfehlungen.

### Was ist neu?

#### 1. Intelligentes Scoring-System (Backend)
- **5 Scoring-Algorithmen** mit 31 Unit-Tests (alle ✓):
  - Workload-Score (70-90% Auslastung = optimal)
  - Compliance-Score (ArbZG §3: 48h/Woche, §5: 11h Ruhezeit)
...
- `ComplianceViolation` - ArbZG-Verstöße mit Severity-Level (WARNING/ERROR/CRITICAL)
...
- 🟢 **OPTIMAL** (85-100): Perfekter Match, keine Probleme
- 🟡 **GOOD** (70-84): Gute Wahl mit kleineren Einschränkungen
- 🟠 **ACCEPTABLE** (50-69): Akzeptabel, aber nicht ideal
- 🔴 **NOT_RECOMMENDED** (<50): Kritische Compliance-Verstöße

## 🐛 Kritischer Bugfix: Login-Problem
...
**Lösung**:
1. Backend vollständig zu Docker migriert (Container: `sicherheitsdienst-api`)
2. DATABASE_URL: `localhost` → `db:5432` (Docker-Service-Name)
3. Backend hört auf `0.0.0.0` (externe Erreichbarkeit)
4. CORS für externe IP konfiguriert (`http://37.114.53.56:5173`)
...
- **56 Dateien geändert**: 8463 Zeilen hinzugefügt, 403 gelöscht
- **31 Unit-Tests**: Alle ✓ (Scoring-Algorithmen)
...
### Troubleshooting
- `docs/TROUBLESHOOTING_LOGIN.md` - Login-Problem Diagnose & Lösungen

### Seeds & Testing
- `backend/prisma/seeds/README.md` - Seed-Anleitung für v1.8.0
...
1. **Backend Port**: 3000 → 3001 (Docker-Compose-Default)
2. **Frontend API-URL**: Muss explizit auf Port 3001 gesetzt werden
3. **Testdaten**: Nach Docker-Migration müssen Seeds neu ausgeführt werden
...
## 🚀 Nächste Schritte (v1.9.0)

### Urgent (heute Abend testen)
- [ ] Testdaten wiederherstellen (Seeds in Docker ausführen)
...
- [ ] E2E-Test: Kompletter Workflow (Abwesenheit → Ersatz finden → Zuweisen)

### Mittelfristig
- [ ] Integration-Tests für v2 API-Endpoint
- [ ] Performance-Test: Scoring < 500ms
- [ ] Mitarbeiter-Präferenzen-Editor UI (`/employees/:id/preferences`)
- [ ] Workload-Dashboard für Mitarbeiter (`/employees/:id/workload`)
- [ ] Team-Fairness-Übersicht (`/team/fairness`)

### Langfristig (v2.0)
- [ ] Cron-Jobs für automatische Workload-Berechnung
...
### Was gut lief
1. **Systematisches Vorgehen**: Phasen 2a → 2b → 2c funktionierte perfekt
...
3. **Seed-Daten verloren**: Nach Docker-Migration müssen alle Seeds neu gelaufen werden

### Verbesserungen für nächstes Mal
1. **Backup vor Infrastruktur-Änderungen**: Testdaten sichern
...
**Status**: ✅ Released
**Nächster Meilenstein**: v1.9.0 (Mitarbeiter-Präferenzen-Editor)
**Kontakt**: GitHub Issues für Bug-Reports & Feature-Requests
